...
static qboolean BotAimsAtSomething (bot_t &pBot)
{
   ...
   const qboolean visible = FPredictedVisible(pBot);
...
static void BotPointGun(bot_t &pBot)
{
   ...
   else if(BotAimsAtSomething (pBot))
   {
	  ...
	  const bot_weapon_select_t * pSelect = GetWeaponSelect(pBot.current_weapon.iId);
	  if(pSelect)
...
{   
   ...
   {
	   const float old_angle = pBot.pEdict->v.v_angle.z;
...
{
	const float delay_min = skill_settings[pBot.bot_skill].react_delay_min;
	const float delay_max = skill_settings[pBot.bot_skill].react_delay_max;
...
void free_posdata_list(int idx)
{
   memset(players[idx].posdata_mem, 0, sizeof(players[idx].posdata_mem));
	...
	players[idx].position_oldest = nullptr;
	players[idx].position_latest = nullptr;
}
...
static void timetrim_posdata(int idx) 
{
   ...
   if(!players[idx].position_oldest) 
   {
	   JKASSERT(players[idx].position_latest != nullptr);
...
static Vector AddPredictionPositionVaritation(const bot_t& pBot)
{
   ...
   v_rnd_angles.x = UTIL_WrapAngle(RANDOM_FLOAT2(-90.0f, 90.0f));
   v_rnd_angles.y = UTIL_WrapAngle(RANDOM_FLOAT2(-180.0f, 180.0f));
   v_rnd_angles.z = 0;
									 ...
									 const Vector& cv_velocity, float time, qboolean ducking, qboolean without_velocity)
{
   ...
   TraceResult tr;
   UTIL_TraceHull( v_src, v_dest, ignore_monsters, (ducking) ? head_hull : human_hull, pPlayer ? pPlayer->v.pContainingEntity : nullptr, &tr);
...
static Vector GetPredictedPlayerPosition(bot_t &pBot, edict_t * pPlayer, qboolean without_velocity = FALSE) 
{
   ...
   // find position data slots that are around 'time'
	const posdata_t* newer = players[idx].position_latest;
	const posdata_t* older = nullptr;
   while(newer) 
   {
	  if(newer->time > time) 
	  {
   ...
   if(!newer) 
   {
	  memset(&newertmp, 0, sizeof(newertmp)); //TODO: replace memset with constructor
   ...
   const Vector pred_origin = (older_diff/total_diff) * newer->origin + (newer_diff/total_diff) * older->origin;
...
qboolean FPredictedVisible(bot_t &pBot)
{
   ...
   const Vector v_enemy = GetPredictedPlayerPosition(pBot, pBot.pBotEnemy, TRUE); //only get position
...
static qboolean FCanShootInHead(edict_t * pEdict, edict_t * pTarget, const Vector & v_dest)
{
   ...
   const float neg = pTarget->v.view_ofs.z >= 2.0f ? 2.0f : 0.0f;
   ...
   const double distance = (v_dest - pEdict->v.origin).Length();
...
static edict_t *BotFindEnemyNearestToPoint(const bot_t& pBot, const Vector& v_point, float radius, Vector* v_found)
{
   ...
   for (int i = 1; i <= gpGlobals->maxClients; i++)
   {
	  ...
	  if ((pPlayer) && (!pPlayer->free) && (pPlayer != pBotEdict) && !FBitSet(pPlayer->v.flags, FL_PROXY))
	  {
		 ...
		 const double distance = (v_origin - v_point).Length();
...
static edict_t *BotFindVisibleSoundEnemy( bot_t &pBot )
{
   ...
   for(int iSound = CSoundEnt::ActiveList(); iSound != SOUNDLIST_EMPTY; iSound = pCurrentSound->m_iNext)
   {
	  ...
	  const double s_distance = (pCurrentSound->m_vecOrigin - pEdict->v.origin).Length();
	  if(s_distance > pCurrentSound->m_iVolume * BotGetHearingSensitivity(pBot))
		 continue;

	  // more distant than what we got already?
	  const double distance = (pCurrentSound->m_vecOrigin - pEdict->v.origin).Length();
...
void BotRemoveEnemy( bot_t &pBot, qboolean b_keep_tracking )
{
   ...
   // track enemy?
   if(pBot.pBotEnemy != nullptr && b_keep_tracking)
   {
	  ...
	  {
		  const int waypoint = WaypointFindNearest(pBot.pTrackSoundEdict, 1024.0f);
   ...
   pBot.curr_waypoint_index = WaypointFindNearest(pEdict, 1024.0f);
}
...
edict_t *pEdict = pBot.pEdict;
   ...
   if (pBot.pBotEnemy != nullptr)  // does the bot already have an enemy?
   {
	  ...
	  else  // enemy is still alive
	  {
		 ...
		 if( pBot.f_bot_see_enemy_time > 0 && pBot.f_bot_see_enemy_time + 0.5f >= gpGlobals->time)
		 {
   ...
   if (pNewEnemy == nullptr)
   {
	  ...
	  pBreakable = nullptr;
	  while((pBreakable = UTIL_FindBreakable(pBreakable)) != nullptr) 
	  {
		 // removed? null?
		 ...
		 // cannot take damage
		 if(static_cast<int>(pBreakable->pEdict->v.takedamage) == DAMAGE_NO ||
		...
		continue; // skip breakables with large health
		 ...
		 double distance = GetModifiedEnemyDistance(pBot, v_origin - pEdict->v.origin).Length();
		 if (distance >= nearestdistance)
			continue;
	 ...
	 enemy_type = info;
	  ...
	  {
		 if (!(pMonster->v.flags & FL_MONSTER) || static_cast<int>(pMonster->v.takedamage) == DAMAGE_NO)
		...
		continue; // skip monsters with large health
		 ...
		 double distance = GetModifiedEnemyDistance(pBot, UTIL_GetOriginWithExtent(pBot, pMonster) - pEdict->v.origin).Length();
		 if (distance >= nearestdistance)
			continue;
	 ...
	 enemy_type = info;
	  ...
	  for (i = 1; i <= gpGlobals->maxClients; i++)
	  {
		 ...
		 if ((pPlayer) && (!pPlayer->free) && (pPlayer != pEdict) && !FBitSet(pPlayer->v.flags, FL_PROXY))
		 {
			...
			double distance = GetModifiedEnemyDistance(pBot, UTIL_GetOriginWithExtent(pBot, pPlayer) - pEdict->v.origin).Length();
			if (distance >= nearestdistance)
			   continue;
...
static qboolean HaveRoomForThrow(const bot_t& pBot)
{
   ...
   //feet
   Vector v_start = pEdict->v.origin - pEdict->v.view_ofs;
...
static qboolean BotFireWeapon(const Vector & v_enemy, bot_t &pBot, int weapon_choice)
{
   ...
   while (pSelect[++select_index].iId)
   {
	  ...
	  select_list_t *next = static_cast<select_list_t*>(alloca(sizeof(select_list_t)));
	  next->next = nullptr;
...
void BotShootAtEnemy( bot_t &pBot )
{
   ...
   v_enemy_xy.z = 0;  // ignore z component (up & down)
	...
	const double f_xy_distance = v_enemy_xy.Length();  // how far away is the enemy scum?
   ...
   if (pBot.f_shoot_time <= gpGlobals->time)
   {
	  ...
	  {
		  const float shootcone_diameter = skill_settings[pBot.bot_skill].shootcone_diameter;
		  const float shootcone_minangle = skill_settings[pBot.bot_skill].shootcone_minangle;
...
qboolean BotShootTripmine( bot_t &pBot )
{
   ...
   const Vector enemy_angle = UTIL_VecToAngles(v_enemy);
