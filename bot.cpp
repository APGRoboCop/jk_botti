...
{
	pBot.bot_think_time = -1.0f;
	pBot.f_last_think_time = gpGlobals->time - 0.1f;
	...
	pBot.f_last_item_found = 0.0f;

	pBot.pTrackSoundEdict = nullptr;
	pBot.f_track_sound_time = 0.0f;
	...
	pBot.f_random_jump_time = 0.0f;
	pBot.f_random_jump_duck_time = 0.0f;
	pBot.f_random_jump_duck_end = 0.0f;
	pBot.f_random_duck_time = 0.0f;
	...
	pBot.current_opt_distance = 99999.0f;
...
static void BotPickLogo(bot_t& pBot)
{
	...
	int logo_index = RANDOM_LONG2(1, num_logos) - 1;  // zero based
	...
	while ((used) && (check_count < MAX_BOT_LOGOS))
	{
		...
		for (const auto& bot : bots)
		{
			if (bot.is_used)
			{
				if (strcmp(bot.logo_name, bot_logos[logo_index]) == 0)
				{
					used = TRUE;
				}
			}
...
static void BotPickName(char* name_buffer, int sizeof_name_buffer)
{
	...
	int name_index = RANDOM_LONG2(1, number_names) - 1;  // zero based
...
static qboolean TeamInTeamBlockList(const char* teamname)
{
	...
	const unsigned int slen = strlen(team_blockedlist);
	char* blocklist = static_cast<char*>(alloca(slen + 1));
	memcpy(blocklist, team_blockedlist, slen + 1);

	const char* pName = strtok(blocklist, ";");
...
static void RecountTeams()
{
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		const char* pTeamName = UTIL_GetTeam(plr, teamname, sizeof(teamname));
		...
		if (tm >= 0)
		{
			g_team_scores[tm] += static_cast<int>(plr->v.frags);
		}
	}
}
...
{
	const float f_team_play = CVAR_GET_FLOAT("mp_teamplay");  // teamplay enabled?
...
void BotCreate(const char* skin, const char* name, int skill, int top_color, int bottom_color, int cfg_bot_index)
{
	...
	else
	{
		...
		pBot = bot_t(); // Assuming bot_t has a default constructor that initializes its members.
		...
		pBot.stay_time = 60 * static_cast<double>(RANDOM_FLOAT2(30, 160));
		// bot has been already here for 10%-50% of the total stay time
		pBot.connect_time = UTIL_GetSecs() - pBot.stay_time * static_cast<double>(RANDOM_FLOAT2(0.2f, 0.8f));
	}
}
...
void BotReplaceConnectionTime(const char* name, float* timeslot)
{
	...
	{
		// find bot by name
		if (strcmp(p_bot.name, name) != 0)
			continue;

		const double current_time = UTIL_GetSecs();

		// check if stay time has been exceeded
		if (current_time - p_bot.connect_time > p_bot.stay_time || current_time - p_bot.connect_time <= 0)
		{
			// use system wide timer for connection times
			// bot will stay 30-160 minutes
			p_bot.stay_time = 60 * static_cast<double>(RANDOM_FLOAT2(30, 160));
			// bot has been already here for 20%-80% of the total stay time
			p_bot.connect_time = current_time - p_bot.stay_time * static_cast<double>(RANDOM_FLOAT2(0.2f, 0.8f));
		}

		*timeslot = static_cast<float>(current_time - p_bot.connect_time);
...
static void BotFindItem(bot_t& pBot)
{
	...
	if (pBot.f_last_item_found > 0 && pBot.f_last_item_found + 5.0f < gpGlobals->time)
	{
	...
	while ((pent = UTIL_FindEntityInSphere(pent, pEdict->v.origin, radius)) != nullptr)
	{
		...
		if (strncmp("func_", item_name, 5) == 0)
		{
			...
			if ((strcmp("func_ladder", item_name) == 0) && (num_waypoints == 0))
			{
				...
				if (tr.flFraction > 0.999999f)
				{
					...
					double distance = (vecEnd - vecStart).Length();
		...
		else  // everything else...
		{
			...
			if (BotEntityIsVisible(pBot, vecEnd))
			{
				...
				{
					double distance = (pent->v.origin - pEdict->v.origin).Length();
	...
	endloop:
		...
		{
			double distance = (entity_origin - pEdict->v.origin).Length();
...
static qboolean BotLookForGrenades(bot_t& pBot)
{
	...
	while ((pent = UTIL_FindEntityInSphere(pent, pEdict->v.origin, radius)) != nullptr)
	{
		Vector entity_origin = UTIL_GetOriginWithExtent(pBot, pent);
...
static void BotCheckLogoSpraying(bot_t& pBot)
{
	...
	if (pBot.b_spray_logo)  // trying to spray a logo?
	{
		...
		Vector v_dest = v_src + UTIL_AnglesToForward(angle) * 100;
		...
		if (tr.flFraction < 1.0f)
		{
			...
			pEdict->v.idealpitch = RANDOM_FLOAT2(0.0f, 30.0f) - 15.0f;
		}
		else
		{
			...
			if (tr.flFraction < 1.0f)
			{
				...
				const Vector bot_angles = UTIL_VecToAngles(v_dest - v_src);
...
static void BotJustWanderAround(bot_t& pBot, float moved_distance)
{
	...
	if (pBot.b_see_tripmine && !FNullEnt(pBot.tripmine_edict))
	{
		...
		{
			UTIL_VecToAngles(pBot.v_tripmine - pEdict->v.origin);
	...
	else if (pBot.b_use_health_station)
	{
		if ((pBot.f_use_health_time + 10.0f) > gpGlobals->time)
		{
			...
			// aim at the button..
			const Vector v_target = pBot.v_use_target - GetGunPosition(pEdict);
			const Vector target_angle = UTIL_VecToAngles(v_target);
	...
	else if (pBot.b_use_HEV_station)
	{
		...
		{
			pBot.f_move_speed = 0.0f;  // don't move while using HEV station
	...
	else
	{
		...
		if (found_waypoint == FALSE)
		{
			...
			if (pBot.f_dont_avoid_wall_time <= gpGlobals->time)
			{
				...
				{
					const Vector v_forward = UTIL_AnglesToForward(pEdict->v.v_angle);
...
static void BotDoStrafe(bot_t& pBot)
{
	...
	// combat strafe
	if (pBot.pBotEnemy != nullptr && FInViewCone(UTIL_GetOriginWithExtent(pBot, pBot.pBotEnemy), pEdict) && FVisibleEnemy(UTIL_GetOriginWithExtent(pBot, pBot.pBotEnemy), pEdict, pBot.pBotEnemy))
	{
		...
		if (RANDOM_LONG2(1, 100) <= skill_settings[pBot.bot_skill].battle_strafe)
		{
			...
			{
				pBot.f_strafe_time = gpGlobals->time + RANDOM_FLOAT2(0.5f, 1.2f);
				pBot.f_strafe_direction = (RANDOM_LONG2(1, 100) <= 50) ? 1.0f : -1.0f;
				pBot.f_move_direction = (RANDOM_LONG2(1, 100) <= 50) ? 1.0f : -1.0f;
			}

			pBot.f_move_speed = 1.0f;
		}
		else
		{
			if (pBot.f_strafe_time <= gpGlobals->time)
			{
				pBot.f_strafe_time = gpGlobals->time + RANDOM_FLOAT2(0.5f, 1.2f);
				pBot.f_strafe_direction = RANDOM_FLOAT(-0.3f, 0.3f);
	...
	else if (pBot.b_on_ladder)
	{
		...
		if (pBot.curr_waypoint_index != -1 && waypoints[pBot.curr_waypoint_index].flags & W_FL_LADDER)
		{
			if (pEdict->v.velocity.Make2D().Length() > 20.0f)
			{
				...
				const float move_dir = UTIL_VecToAngles(pEdict->v.velocity).y;
				const float look_dir = pEdict->v.v_angle.y;

				const float angle_diff = UTIL_WrapAngle(move_dir - look_dir);
			...
			else
			{
				...
				const Vector v_wp = waypoints[pBot.curr_waypoint_index].origin;
				const Vector v_dir = (v_wp - pEdict->v.origin);

				const Vector v_angles = UTIL_VecToAngles(v_dir);

				const float waypoint_dir = v_angles.y;
				const float look_dir = pEdict->v.v_angle.y;

				const float angle_diff = UTIL_WrapAngle(waypoint_dir - look_dir);
	...
	{
		pBot.f_strafe_time = gpGlobals->time + RANDOM_FLOAT2(0.1f, 1.0f);
		...
		if (RANDOM_LONG2(1, 100) <= skill_settings[pBot.bot_skill].normal_strafe)
		{
			...
			pBot.f_strafe_direction = 0.0f + RANDOM_FLOAT2(-0.005f, 0.005f);
...
static void BotDoRandomJumpingAndDuckingAndLongJumping(bot_t& pBot, float moved_distance)
{
	...
	if (pBot.b_longjump_do_jump)
	{
		...
		pBot.f_duck_time = gpGlobals->time + RANDOM_FLOAT2(0.3f, 0.5f); //stay duck for some time to fix animation
	...
	if (pBot.f_random_jump_duck_time > 0.0f && pBot.f_random_jump_duck_time <= gpGlobals->time)
	{
		...
		if (pBot.f_random_jump_duck_end > 0.0f && pBot.f_random_jump_duck_end <= gpGlobals->time)
		{
			pBot.f_random_jump_duck_time = 0.0f;
			pBot.f_random_jump_duck_end = 0.0f;
			pBot.f_random_jump_time = gpGlobals->time + RANDOM_FLOAT2(0.2f, 0.4f);
		}
		...
		std::fabs(pEdict->v.v_angle.y - pEdict->v.ideal_yaw) <= 10)
	{
		...
		if (RANDOM_LONG2(1, 100) > 10)
			pBot.f_combat_longjump = gpGlobals->time + 1.0f;
		else
			pBot.f_combat_longjump = gpGlobals->time + RANDOM_FLOAT2(3.0f, 5.0f);
	...
	if (pBot.pBotEnemy == nullptr && !FNullEnt(pBot.pBotPickupItem) && !(pBot.pBotPickupItem->v.effects & EF_NODRAW) && pBot.pBotPickupItem->v.frame <= 0)
		return;

	// if in combat mode jump more
	qboolean jump = (pBot.prev_random_type != 1 && pBot.f_random_jump_time <= gpGlobals->time &&
		(moved_distance >= 10.0f || pBot.pBotEnemy != nullptr) && pBot.f_move_speed > 1.0f &&
		RANDOM_LONG2(1, 100) <= skill_settings[pBot.bot_skill].random_jump_frequency);

	qboolean duck = (pBot.prev_random_type != 2 && pBot.f_random_duck_time <= gpGlobals->time &&
		pBot.pBotEnemy != nullptr &&
	...
	if (lj)
	{
		...
		for (int i = 1; i >= -1; i -= 2)
		{
			...
			target_angle.y = UTIL_WrapAngle(pEdict->v.v_angle.y + 30.0f * (mod * i));
		...
		pBot.f_combat_longjump = gpGlobals->time + RANDOM_FLOAT2(0.3f, 0.6f); // don't try too often
...
void BotThink(bot_t& pBot)
{
	...
	pBot.msecval = pBot.f_frame_time * 1000.0f;
	...
	if (g_in_intermission)
	{
		...
		BotRunPlayerMove(pBot, pEdict->v.v_angle, 0, 0, 0, 0, 0, static_cast<byte>(pBot.msecval));
		return;
	}
	...
	if ((pEdict->v.health < 1) || (pEdict->v.deadflag != DEAD_NO))
	{
		...
		BotRunPlayerMove(pBot, pEdict->v.v_angle, pBot.f_move_speed, 0, 0, pEdict->v.button, 0, static_cast<byte>(pBot.msecval));
	...
	if (pBot.curr_waypoint_index != -1)
	{
		...
		if (waypoints[pBot.curr_waypoint_index].flags & W_FL_LADDER)
		{
			...
			if (pBot.waypoint_top_of_ladder && !pBot.b_on_ladder)
			{
				...
				if (pBot.b_on_ground)
				{
					double waypoint_distance = (pEdict->v.origin - pBot.waypoint_origin).Length();
	...
	BotRunPlayerMove(pBot, pEdict->v.v_angle, pBot.f_move_speed,
		f_strafe_speed, 0, pEdict->v.button, 0, static_cast<byte>(pBot.msecval));
