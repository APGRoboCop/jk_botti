...
static playermove_s *old_ppmove = nullptr;

cvar_t jk_botti_version = { "jk_botti_version", "", FCVAR_EXTDLL|FCVAR_SERVER, 0, nullptr};


static int CheckSubMod()
{
   int submod = 0;
   ...
   if(!strnicmp(desc, "Opposing", 8) || !strnicmp(desc, "OpFor", 5))
	  ...
	  submod = SUBMOD_XDM;
   else if(CVAR_GET_POINTER("bm_ver") != nullptr)
   ...
   else if(!strnicmp(desc, "HL Teamplay", 11))
   {
	  ...
	  if(CVAR_GET_POINTER("mp_giveweapons") != nullptr && CVAR_GET_POINTER("mp_giveammo") != nullptr)
		 submod = SUBMOD_SEVS;
	  else
		 submod = SUBMOD_HLDM;
   }
...
static int Spawn_Post( edict_t *pent )
{
   ...
   else if(FIsClassname(pent, "func_plat"))
   {
	  ...
	  const Vector v_position1 = m_origin;
   ...
   else if(FIsClassname(pent, "func_door"))
   {
	  ...
	  {
		  const Vector swap = v_position1;
		 v_position1 = v_position2;
		 v_position2 = swap;
	  }

	  const Vector start = v_position1;
	  const Vector end = v_position2;
...
{ 
   ...
   {
	  // don't try to add bots for 10 second, give client time to get added
	  bot_check_time = gpGlobals->time + 10.0f;
   }
...
static void CmdStart( const edict_t *player, const usercmd_s *cmd, unsigned int random_seed )
{
   // check if is our bot
   ...
   jkbotti_ClientConnect(const_cast<edict_t*>(player), STRING(player->v.netname), "::::local:other_bot", szRejectReason);
   jkbotti_ClientPutInServer( const_cast<edict_t*>(player) );
   ...
   if (gpGlobals->deathmatch)
   {
	  ...
	  for (auto& bot : bots)
	  {
		 if (bot.pEdict == pEntity)
		 {
			...
			bot.f_kick_time = gpGlobals->time;  // save the kicked time
			bot.userid = 0;
...
static void new_PM_PlaySound(int channel, const char *sample, float volume, float attenuation, int fFlags, int pitch) 
{
   if (gpGlobals->deathmatch)
   {
	  ...
	  {
		 edict_t* pPlayer = INDEXENT(idx + 1);
   
		 if(!FNullEnt(pPlayer))
		 {
			 const int ivolume = static_cast<int>(1000 * ((volume + 1) / 2));
...
static void PM_Move(playermove_s *ppmove, qboolean server) 
{
   if (!gpGlobals->deathmatch) 
	  RETURN_META(MRES_IGNORED);
...
static void StartFrame()
{
   ...
   if (need_to_open_cfg)  // have we open jk_botti.cfg file yet?
   {
	  ...
	  {
		 UTIL_BuildFileName_N(filename, sizeof(filename), "addons/jk_botti/jk_botti.cfg", nullptr);
...
plugin_info_t Plugin_info = {
   ...
   "github.com/Bots-United/jk_botti", // plugin URL
...
C_DLLEXPORT int Meta_Attach (PLUG_LOADTIME now, META_FUNCTIONS *pFunctionTable, meta_globals_t *pMGlobals, gamedll_funcs_t *pGamedllFuncs)
{
   ...
   // init random
   fast_random_seed(time(nullptr) ^ reinterpret_cast<unsigned long>(&bots[0]) ^ sizeof(bots));
...
C_DLLEXPORT int Meta_Detach (PLUG_LOADTIME now, PL_UNLOAD_REASON reason)
{
   ...
   for (auto& bot : bots)
	   if (bot.is_used)  // is this slot used?
		 BotKick(bot);
