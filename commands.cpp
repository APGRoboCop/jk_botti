...
int AddToCfgBotRecord(const char *skin, const char *name, int skill, int top_color, int bottom_color)
{
   ...
   cfg_bot_record = static_cast<cfg_bot_record_t*>(realloc(cfg_bot_record, sizeof(cfg_bot_record_t) * cfg_bot_record_size));
   ...
   else if (FStrEq(pcmd, "botthinkfps"))
   {
	  if ((arg1 != nullptr) && (*arg1 != 0))
	  {
		 ...
		 if ((temp < 1) || (temp > 100))
			...
			bot_think_spf = 1.0f / static_cast<float>(temp);
	  }
